...
export class OcEditUserTypeService {
  ...
  constructor(private organizationTypeService: DeveloperTypeService,
  ...
  private getDeveloperTypes(injectOrganizationType: boolean, configs: OcEditUserFormConfig [])
    ...
    if (injectOrganizationType) {
      ...
      const searchQuery = orgTypesIDs?.length > 0 ?
          `{'developerTypeId':{'$in': ['${orgTypesIDs.join('\',\'')}']}}` : '';
      if (searchQuery) {
        return this.organizationTypeService.getAllDeveloperTypes(
             1, 100, searchQuery);
      }
    }
  ...
  private getDeveloperAccountTypes(injectAccountType: boolean, configs: OcEditUserFormConfig [])
      : Observable<Page<TypeModel<TypeFieldModel>>> {
